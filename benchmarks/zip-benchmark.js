...
global.parallelWork = function(writer, packOptions, parallelism, cb) {
  var finished = _.after(parallelism, cb);

  for (var i = 0; i < parallelism; i++) {
    work(writer, {}, finished);
  }
};

// Individual file pack & write
global.work = function(writer, packOptions, cb) {
  writer.pack(packOptions);
  cb();
};
...
module.exports = {
  ...
  tests: {
    ...
    'Small dataset - Packing (no compression)': {
      ...
      fn: function(deferred){
        work(writer, {compression: 'STORE'}, function(){
          deferred.resolve();
        });
      }
    },
    'Small dataset - Packing (parallelism: 10)': {
      ...
      fn: function(deferred){
        var parallelism = 10;
        parallelWork(writer, {}, 10, function(){
          deferred.resolve();
        });
      }
    },
